...
{
    public class Recipe_SurgeryMechanical : RecipeWorker
    {
        ...
        protected bool CheckSurgeryFailMechanical(Pawn surgeon, Pawn patient, List<Thing> ingredients, BodyPartRecord part, Bill bill)
        { // Check if the current surgery will fail.
            ...
            chanceSucceed *= surgeon.GetStatValue(MHC_StatDefOf.MHC_MechanicalSurgerySuccessChance);

            // Multiply success chance by quality of bed
            if (!recipe.surgeryIgnoreEnvironment && patient.InBed())
            {
                chanceSucceed *= patient.CurrentBed().GetStatValue(MHC_StatDefOf.MHC_MechanicalSurgerySuccessChanceFactor);
            }
            else
            ...
            chanceSucceed = Mathf.Min(chanceSucceed, MechHumanlikes_Settings.maxChanceMechanicOperationSuccess);
            ...
            { // Surgery failed. Determine the extent of the failure.
                ...
                { // Patient died due to the surgery. This kills them for good.
                    ...
                    Messages.Message("MHC_MessageMechanicOperationFailureFatal".Translate(surgeon.LabelShort, patient.LabelShort, recipe.label), patient, MessageTypeDefOf.NegativeHealthEvent);
                }
                else if (Rand.Chance(MechHumanlikes_Settings.chanceFailedOperationMinor))
                { // Patient suffered minor injuries.
                    Messages.Message("MHC_MessageMechanicOperationFailureMinor".Translate(surgeon.LabelShort, patient.LabelShort), patient, MessageTypeDefOf.NegativeHealthEvent);
                    HealthUtility.GiveRandomSurgeryInjuries(patient, 10, part);
                }
                else
                { // Patient suffered major injuries.
                    Messages.Message("MHC_MessageMechanicOperationFailureRidiculous".Translate(surgeon.LabelShort, patient.LabelShort), patient, MessageTypeDefOf.NegativeHealthEvent);
